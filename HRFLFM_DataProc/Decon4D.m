function [PSFpath,FLFMpath] = Decon4D(PSFpath,FLFMpath,Reconpath,Iter)
...
Spectral_Size = size(FLPSF,4);
disp(['PSF [',PSFpath,'] read!',' Total layers: ',num2str(Depth_Size), ' Total spectral layers: ',num2str(Spectral_Size)])
%% load BP PSF
...
% n = 10;
% if isempty(OTF)
OTF = zeros(size(zeroImageEx,1), size(zeroImageEx,2), size(FLFPSF,3), size(FLFPSF,4),'single');
INVOTF = zeros(size(zeroImageEx,1), size(zeroImageEx,2), size(FLFPSF,3), size(FLFPSF,4),'single');
for dd = 1:size(FLFPSF,4)
    for cc = 1:size(FLFPSF,3)
        OTF(:,:,cc,dd) = getOTFcpu(FLFPSF(:,:,cc,dd));
        INVOTF(:,:,cc,dd) = getOTFcpu(imrotate(FLFPSF(:,:,cc,dd),180));
...
% end
for jj = 1:1:size(OTF,4)
    for ii = 1:1:size(OTF,3)
        OTF(:,:,ii,jj) = OTF(:,:,ii,jj)/max(max(OTF(:,:,ii,jj)));
        INVOTF(:,:,ii,jj) = INVOTF(:,:,ii)/max(max(INVOTF(:,:,ii,jj)));
    end
end
