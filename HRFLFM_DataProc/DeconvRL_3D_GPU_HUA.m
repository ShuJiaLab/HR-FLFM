function Xguess = DeconvRL_3D_GPU_HUA( OTF,INVOTF, maxIter, FLFimg )
...
BackProj = gpuArray.zeros(FLFsize(1), FLFsize(2), size(INVOTF,3) , 'single');
% time0 = toc;
for cc = 1:size(INVOTF,3)   
    tempOTF = gpuArray(INVOTF(:,:,cc));
    tempProj = real(ifft2(tempOTF.*fft2(FLFimg)));
    tempProj = tempProj(1:FLFsize(1),1:FLFsize(2));
    BackProj(:,:,cc) = tempProj;
end
% disp(toc - time0)
end
...
for cc = 1:size(OTF,3) 
    tempOTF = gpuArray(OTF(:,:,cc));
    OBJpad = zeroPad(OBJimg(:,:,cc),zeroImageEx);
